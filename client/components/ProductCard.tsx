...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    const isLiked = false; // This should be replaced with actual logic to determine if the product is liked by the user
    ...
    return (
        <Link href={`/product/${product._id}`} asChild>
            <TouchableOpacity className='w-[48%] mb-4 bg-white rounded-lg overflow-hidden'>
                ...
                <Text className='text-sm mb-1 font-medium text-primary' numberOfLines={1}>{product.name}</Text>
